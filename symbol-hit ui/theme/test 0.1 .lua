...
do
    local function initServices()
        ...
        function cache.getPing()
            ...
            if item then
                local ok, v = pcall(function() return item:GetValueString() end)
                if ok and v then
...
local Library = (function()
        ...
        Data = {
            ...
            ['Apocalypse Rising 2'] = {
                ...
                GetPlayers = function()
                    ...
                    if not Zombies then return Entities end
                    local Mobs = Zombies:FindFirstChild('Mobs')
                    if not Mobs then return Entities end
    ...
    task.spawn(function()
        ...
        if success1 then
            local jsonSuccess, data = pcall(function() return HttpService:JSONDecode(response1) end)
        ...
        if success2 then
            local jsonSuccess, data = pcall(function() return HttpService:JSONDecode(response2) end)
            if jsonSuccess and type(data) == 'table' then
                for _, file in ipairs(data)do
    ...
    local Fonts = {
        ...
        Names = {'Proggy','ProggyTiny','Minecraftia','SmallestPixel7','Verdana','VerdanaBold','Tahoma','ProggyClean','TahomaXP'},
        Data = {},
    }
    ...
    do
        ...
        function Fonts.Get(font) return Fonts.Data[font] end
    ...
    do
        ...
        function Images.Get(image) return Images.Data[image] end
        for _, image in Images.Names do Images.Load(image) end
    end
    ...
    do
        ...
        function AssetLoader:Init(directory)
            ...
            if not isfolder(self.AssetsFolder) then makefolder(self.AssetsFolder) end
        end
        ...
        function AssetLoader:GetAssetNames() return self.AssetNames end

        function AssetLoader:LoadAll(assetNames)
            if assetNames then
                for _, name in ipairs(assetNames)do self:LoadAsset(name) end
            else
                ...
                for _, filePath in ipairs(files)do
                    ...
                    if fileName ~= '' then self:LoadAsset(fileName) end
                end
            end
        end

        function AssetLoader:PreloadDefaults()
            for _, name in ipairs(self.Names)do self:LoadAsset(name) end
        end
    end
    ...
    local Library = {
        ...
        KeyConverters = {
            ...
            tab = 'TAB', capslock = 'CAP', leftshift = 'LSH', mousebutton1 = 'MB1',
            mousebutton2 = 'MB2', mousebutton3 = 'MB3', rightshift = 'RSH',
            leftcontrol = 'CTRL', leftalt = 'ALT', leftsuper = 'WIN',
            rightcontrol = 'CTRL', rightalt = 'ALT', rightsuper = 'WIN',
            insert = 'INS', delete = 'DEL', home = 'HME', pageup = 'PUD',
            pagedown = 'PDN', up = 'UP', down = 'DWN', left = 'LFT', right = 'RGT',
    ...
    do
        ...
        do
            function Utility.New(object, props, theme)
                ...
                if props then
                    for prop, val in props do Obj[prop] = val end
                end
                if theme then Library.AddObjectTheme(Obj, theme) end
        ...
        -- FIXED: DisplayOrder + IgnoreGuiInset so watermark/KBL render above everything
        ...
        function Library.ChangeObjectTheme(object, props, tweened)
            ...
            if Theme then
                ...
                for prop, v in props do
                    if type(v) == 'string' then
                        if tweened then
                            Theme.Tween = Library.Tween(object, { [prop] = Library.Theme[v] })
                        else
                            object[prop] = Library.Theme[v]
                        end
                    elseif type(v) == 'function' then
        ...
        function Library.UpdateTheme(theme, color)
            ...
            for _, themeobj in Library.ThemeObjects do
                for prop, val in themeobj.Props do
                    if val == theme then
                        if themeobj.Tween then themeobj.Tween:Cancel() end
            ...
            if theme == 'Accent' then
                for _, obj in pairs(Utility.Objects) do
                    if obj.Name == 'ToggleIcon' and obj:IsA('ImageLabel') then
                        if obj.Visible then obj.ImageColor3 = color end
                    end
                end
            end
        end
        ...
        function Library.Config(cfg, default)
            local Table = {}
            for name, val in cfg do Table[name:lower()] = val end
            for name, val in default do
                if Table[name] == nil then Table[name] = val end
            end
            return Table
        end
        ...
        -- Generic drag helper
        function Library.StartDragging(holder, handle, state)
            Utility.Signal(handle.InputBegan:Connect(function(input)
                if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
                state.Dragging = true
                state.DragStart = input.Position
                state.DragStartPos = holder.AbsolutePosition
            end))
            Utility.Signal(UserInputService.InputChanged:Connect(function(input)
                ...
                holder.Position = UDim2.new(
                    0, state.DragStartPos.X + Delta.X,
                    0, state.DragStartPos.Y + Delta.Y
                )
            end))
            Utility.Signal(UserInputService.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        function Library.ColorpickerWindow(self)
            ...
            do
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding', Parent = Objects.Background,
            ...
            function Popup.Open(position)
                ...
                if Popup.Visible then Objects.Outline.Visible = true end
                ...
                for _, obj in ParentObjects do
                    ...
                    if Index then
                        if type(Index) == 'table' then
                            for _, prop in Index do Library.Fade(obj, prop, Popup.Visible) end
                        else
                ...
                if position then Objects.Outline.Position = UDim2.new(0, position.X, 0, position.Y) end
        ...
        function Library.Window(self, cfg)
            ...
            do
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.SideContainer, CornerRadius = UDim.new(0, 5) })
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding',
                    PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5),
                    PaddingTop = UDim.new(0, 8), PaddingBottom = UDim.new(0, 5),
            ...
            function Window.Open()
                ...
                for _, popup in Library.Popups do
                    if popup.Objects.Outline.Visible then popup.Visible(false) end
                end
                for _, dropdown in Library.Dropdowns do
                    if dropdown.Visible then dropdown.Open() end
                end
                ...
                if Window.Visible then Objects.Outline.Visible = true end

                for _, obj in Objects.ScreenGui:GetDescendants()do
                    local Index = Utility.GetTransparency(obj)
                    if Index then
                        if type(Index) == 'table' then
                            for _, prop in Index do Library.Fade(obj, prop, Window.Visible) end
                        else
        ...
        function Library.Info(self, cfg)
            ...
            local Info = { Objects = {}, ZIndex = self.ZIndex }
            ...
            do
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding', Parent = Objects.Holder,
        ...
        function Library.Tab(self, cfg)
            ...
            function Tab.Set(status, nested)
                if self.TabsTweening and status then return end
                if status then
                    Objects.SelectionLine.BackgroundTransparency = 0
                    Library.Tween(Objects.SelectionLine, { BackgroundTransparency = 0 })
                    Library.ChangeObjectTheme(Objects.Text, { TextColor3 = 'Text' }, true)
                    if Objects.Icon then Library.ChangeObjectTheme(Objects.Icon, { ImageColor3 = 'Text' }, true) end
                else
                    Library.Tween(Objects.SelectionLine, { BackgroundTransparency = 1 })
                    Library.ChangeObjectTheme(Objects.Text, { TextColor3 = 'Dark Text' }, true)
                    if Objects.Icon then Library.ChangeObjectTheme(Objects.Icon, { ImageColor3 = 'Dark Text' }, true) end
                end
                ...
                if not self.TabsTweening then
                    if not nested then
                        ...
                        for _, tab in self.Tabs do
                            if tab.Name ~= Tab.Name then tab.Set(false, true) end
                        end
                        for _, obj in Objects.Page:GetDescendants()do
                            local Index = Utility.GetTransparency(obj)
                            if Index then
                                if type(Index) == 'table' then
                                    for _, prop in Index do Library.Fade(obj, prop, status) end
                                else
        ...
        function Library.Section(self, cfg)
            ...
            do
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.MainOutline, CornerRadius = UDim.new(0, cornerRadius) })
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.Outline, CornerRadius = UDim.new(0, cornerRadius) })
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.TitleBackground, CornerRadius = UDim.new(0, 3) })
            ...
            Utility.Signal(Objects.Dragging.MouseButton1Down:Connect(function()
                ...
                local Drag = Utility.Signal(UserInputService.InputChanged:Connect(function(input)
                    if not Section.Dragging or input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
                    ...
                    if ClosestHolder then
                        if not Indicator then
                            ...
                            Utility.New('UICorner', { Name = 'UICorner', Parent = Indicator, CornerRadius = UDim.new(0, 5) })
                            ...
                            Utility.New('UICorner', { Name = 'UICorner', Parent = Background, CornerRadius = UDim.new(0, 5) })
            ...
            function Popup.Visible(visibility, position, stopPropagation)
                ...
                if visibility then
                    ...
                    if not stopPropagation then
                        ...
                        for _, parent in ipairs(parentChain)do parentChainMap[parent] = true end
                ...
                for _, obj in ParentObjects do
                    local Index = Utility.GetTransparency(obj)
                    if type(Index) == 'table' then
                        for _, prop in Index do Library.Fade(obj, prop, visibility) end
                    else
            ...
            if not Library.GlobalPopupClickHandler then
                ...
                Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                    ...
                    local function isLibraryUI(obj)
                        while obj do
                            if obj == Library.ScreenGui then return true end
                            obj = obj.Parent
                        end
                        return false
                    end
                    ...
                    if ignoreCloseCheck then return end

                    for _, popup in pairs(Library.Popups)do
                        if popup.Objects.Outline.Visible then
                            local function isInsidePopupHierarchy(p, input)
                                if Utility.MouseOver(p.Objects.Outline, input) then return true end
                                for _, child in pairs(p.ChildPopups)do
                                    if child.Objects.Outline.Visible and isInsidePopupHierarchy(child, input) then return true end
                                end
                                return false
                            end
                            ...
                            if not isInsidePopupHierarchy(popup, input) then
                                local function isInParentOfPopup(p, input)
                                    if p.ParentPopup and p.ParentPopup.Objects.Outline.Visible then
                                        if Utility.MouseOver(p.ParentPopup.Objects.Outline, input) then return true end
                                ...
                                if not isInParentOfPopup(popup, input) and popup.ParentElement then
                                    ...
                                    if popup.ParentElement.Objects and popup.ParentElement.Objects.Icon then
                                        Library.ChangeObjectTheme(popup.ParentElement.Objects.Icon, { ImageColor3 = 'Dark Text' }, true)
                                    end
                                end
                            end
                        end
        ...
        function Library.Toggle(self, cfg)
            ...
            do
                ...
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Outline })
                ...
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Background })
            ...
            function Toggle.Children(visibility)
                if cfg.children == nil and not Toggle.Holder then return end
                if Toggle.Tweening then return end
                if visibility == Objects.ChildrenHolder.Visible then return end

                Toggle.Tweening = true
                if visibility then Objects.ChildrenHolder.Visible = true end
            ...
            function Toggle.Set(value)
                ...
                if Objects.Icon then
                    ...
                    if value then Objects.Icon.ImageColor3 = Library.Theme.Accent end
                end
        ...
        function Library.Slider(self, cfg)
            ...
            cfg = Library.Config(cfg, {
                name = 'New Slider',
                description = nil,
                value = 50, min = 0, max = 100, float = 1,
                suffix = '%s',
                callback = function() end,
                flag = nil, children = nil,
            })
            if not cfg.flag then cfg.flag = cfg.name end

            local Slider = {
                Tweening = false, Objects = {},
                ZIndex = self.ZIndex, Value = cfg.value, Sliding = false,
            }
            ...
            do
                Objects.Holder = Utility.New('Frame', {
                    Name = 'Holder', Size = UDim2.new(1, 0, 0, 0),
                    Position = UDim2.fromOffset(0, 0), BackgroundTransparency = 1,
                    AutomaticSize = Enum.AutomaticSize.Y, BorderSizePixel = 0,
                    Parent = self.Holder, ZIndex = ZIndex,
                })
                Utility.New('UIPadding', { Parent = Objects.Holder, PaddingBottom = UDim.new(0, 2) })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder, Parent = Objects.Holder,
                    Padding = UDim.new(0, 5),
                })

                ZIndex = ZIndex + 1
                Objects.Line = Utility.New('TextButton', {
                    Name = 'Line', Size = UDim2.new(1, 0, 0, 0),
                    Position = UDim2.fromOffset(0, 0), BackgroundTransparency = 1,
                    AutoButtonColor = false, AutomaticSize = Enum.AutomaticSize.Y,
                    Style = Enum.ButtonStyle.Custom, Text = '',
                    Parent = Objects.Holder, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder, Parent = Objects.Line,
                    Padding = UDim.new(0, 2),
                })

                ZIndex = ZIndex + 1
                Objects.Text = Utility.New('TextLabel', {
                    Name = 'Text', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, Text = cfg.name,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.Line, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Horizontal,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    HorizontalAlignment = Enum.HorizontalAlignment.Right,
                    Parent = Objects.Text, Padding = UDim.new(0, 2),
                })

                ZIndex = ZIndex + 1
                Objects.SideHolder = Utility.New('Frame', {
                    Name = 'SideHolder', Size = UDim2.new(0, 0, 1, 0),
                    AutomaticSize = Enum.AutomaticSize.X, Position = UDim2.fromOffset(0, 0),
                    BackgroundTransparency = 1, BorderSizePixel = 0,
                    Parent = Objects.Text, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Horizontal,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    HorizontalAlignment = Enum.HorizontalAlignment.Right,
                    Parent = Objects.SideHolder, Padding = UDim.new(0, 2),
                })
                Slider.SideHolder = Objects.SideHolder
                ZIndex = ZIndex + 1

                Objects.Value = Utility.New('TextLabel', {
                    Name = 'Value', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(0, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.XY, Text = '',
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.Text, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
                ZIndex = ZIndex + 1
                ...
                if cfg.description then
                    Objects.Description = Utility.New('TextLabel', {
                        Name = 'Description', TextStrokeTransparency = 0.8,
                        BackgroundTransparency = 1, TextSize = Library.FontSize - 2,
                        FontFace = Library.Font, Size = UDim2.new(1, 0, 0, 0),
                        AutomaticSize = Enum.AutomaticSize.Y, Text = cfg.description,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        Parent = Objects.Line, ZIndex = ZIndex, TextWrapped = true,
                    }, { TextColor3 = 'Light Text' })
                    ZIndex = ZIndex + 1
                end

                Objects.Outline = Utility.New('TextButton', {
                    Name = 'Line', Size = UDim2.new(1, 0, 0, 8),
                    Position = UDim2.fromOffset(0, 0), BorderSizePixel = 0,
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Text = '', Parent = Objects.Holder, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })

                Utility.Signal(Objects.Outline.MouseEnter:Connect(function()
                    if not Slider.Sliding then
                        Library.ChangeObjectTheme(Objects.Text, { TextColor3 = 'Accent' }, true)
                    end
                end))
                ...
                ZIndex = ZIndex + 1
                Objects.Background = Utility.New('Frame', {
                    Name = 'Background', Size = UDim2.new(1, -2, 1, -2),
                    Position = UDim2.fromOffset(1, 1), BorderSizePixel = 0,
                    Parent = Objects.Outline, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Background' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Background })

                ZIndex = ZIndex + 1
                Objects.Accent = Utility.New('Frame', {
                    Name = 'Accent', Size = UDim2.new(0, 0, 1, 0),
                    BorderSizePixel = 0, Parent = Objects.Background, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Accent' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Accent })
            end
        ...
        function Library.Button(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Button', confirm = false, autosize = false,
                callback = function() end,
            })

            local Button = {
                Clicked = false, ZIndex = self.ZIndex, Time = 0, Objects = {},
            }
            ...
            do
                Objects.Outline = Utility.New('TextButton', {
                    ...
                    BorderSizePixel = 0, Parent = self.Holder, TextSize = 0, Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Outline })
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding',
                    PaddingTop = UDim.new(0, 2), PaddingBottom = UDim.new(0, 5),
                    PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5),
                    Parent = Objects.Background,
                })
                ...
                Objects.Text = Utility.New('TextLabel', {
                    Name = 'Text',
                    Size = cfg.autosize and UDim2.new(0, 0, 0, 0) or UDim2.new(1, 0, 0, 0),
                    Text = cfg.name, TextSize = Library.FontSize,
                    FontFace = Library.Font, TextXAlignment = Enum.TextXAlignment.Center,
                    BackgroundTransparency = 1, TextStrokeTransparency = 0.8,
                    Parent = Objects.Background, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
        ...
        function Library.List(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New List', description = nil, value = 'value1',
                values = { 'value1', 'value2', 'value3', 'value4', 'value5' },
                multi = false, size = 100, callback = function() end, flag = nil,
            })
            if not cfg.flag then cfg.flag = cfg.name end
            ...
            do
                ...
                Objects.Outline = Utility.New('TextButton', {
                    Name = 'Outline', Size = UDim2.new(1, 0, 0, cfg.size),
                    Position = UDim2.fromOffset(0, 0), BorderSizePixel = 0,
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Text = '', Parent = Objects.Holder, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                ...
                Objects.Background = Utility.New('Frame', {
                    Name = 'Background', Size = UDim2.new(1, -2, 1, -2),
                    Position = UDim2.fromOffset(1, 1), BorderSizePixel = 0,
                    Parent = Objects.Outline, ZIndex = ZIndex, ClipsDescendants = true,
                }, { BackgroundColor3 = 'Background' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Background })
                Utility.New('UIPadding', {
                    Name = 'UIPadding',
                ...
                ZIndex = ZIndex + 1
                Objects.Scrolling = Utility.New('ScrollingFrame', {
                    Name = 'Scrolling', Size = UDim2.new(1, 0, 1, 0),
                    Position = UDim2.fromOffset(0, 0), BackgroundTransparency = 1,
                    BorderSizePixel = 0, Parent = Objects.Background, ZIndex = ZIndex,
                    AutomaticCanvasSize = Enum.AutomaticSize.Y,
                    CanvasSize = UDim2.new(0, 0, 0, 0),
                    BottomImage = Library.ScrollBar, MidImage = Library.ScrollBar,
                    TopImage = Library.ScrollBar, ScrollBarThickness = 2,
                }, { ScrollBarImageColor3 = 'Accent' })
                ZIndex = ZIndex + 1
                Objects.Content = Utility.New('Frame', {
                    Name = 'Content', Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, Position = UDim2.fromOffset(0, 0),
                    BackgroundTransparency = 1, BorderSizePixel = 0,
                    Parent = Objects.Scrolling, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    HorizontalAlignment = Enum.HorizontalAlignment.Center,
                    Parent = Objects.Content, Padding = UDim.new(0, 5),
                })
                ZIndex = ZIndex + 1
            end
            ...
            function List.Set(value)
                if cfg.multi then
                    if type(value) == 'table' then
                        for _, item in List.Items do item.Select(false) end
                        for _, item in value do
                            for _, item2 in List.Items do
                                if item2.Name == item then item2.Select(true) end
                            end
                        end
                        List.Value = value
                        ...
                        if Index then
                            ...
                            for _, item in List.Items do
                                if item.Name == value then item.Select(false) end
                            end
                            cfg.callback(List.Value)
                            Library.Flags[cfg.flag] = List.Value
                        else
                            ...
                            for _, item in List.Items do
                                if item.Name == value then item.Select(true) end
                            end
                            cfg.callback(List.Value)
                            Library.Flags[cfg.flag] = List.Value
                        end
            ...
            function List.Add(name)
                ...
                Objs.Text = Utility.New('TextButton', {
                    ...
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, Text = name,
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.Content, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
                function Item.Select(value)
                    Library.ChangeObjectTheme(Objs.Text, { TextColor3 = value and 'Text' or 'Dark Text' }, true)
                    Item.Selected = value
                end
            ...
            function List.Refresh(tbl)
                for _, item in List.Items do item.Objects.Text:Destroy() end
                List.Items = {}
                List.Value = cfg.multi and {} or nil
                for _, item in tbl do
                    if type(item) == 'string' then List.Add(item) end
                end
            end

            for _, item in cfg.values do List.Add(item) end
            List.Set(cfg.value)
        ...
        function Library.Dropdown(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Dropdown', description = nil,
                values = { 'value1', 'value2', 'value3', 'value4', 'value5' },
                value = 'value1', multi = false, flag = nil,
                callback = function() end,
            })
            if not cfg.flag then cfg.flag = cfg.name end

            local Dropdown = {
                Tweening = false, Visible = false, ZIndex = self.ZIndex,
                Objects = {}, Popup = {}, Items = {}, Value = nil, ParentPopup = self,
            }
            ...
            do
                ...
                Objects.Outline = Utility.New('TextButton', {
                    Name = 'Outline', Size = UDim2.new(1, 0, 0, 20),
                    Position = UDim2.fromOffset(0, 0), BorderSizePixel = 0,
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Text = '', Parent = Objects.Holder, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                ...
                Utility.Signal(Objects.Outline.MouseEnter:Connect(function()
                    if not Dropdown.Visible then
                        Library.ChangeObjectTheme(Objects.Text, { TextColor3 = 'Accent' }, true)
                        Library.ChangeObjectTheme(Objects.Value, { TextColor3 = 'Accent' }, true)
                    end
                end))
                Utility.Signal(Objects.Outline.MouseLeave:Connect(function()
                    if not Dropdown.Visible then
                        Library.ChangeObjectTheme(Objects.Text, { TextColor3 = 'Dark Text' }, true)
                        Library.ChangeObjectTheme(Objects.Value, { TextColor3 = 'Text' }, true)
                    end
                end))
                ...
                Objects.Value = Utility.New('TextLabel', {
                    Name = 'Text', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(1, 0, 1, -2),
                    Text = 'Value', TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.Background, ZIndex = ZIndex,
                }, { TextColor3 = 'Text' })
                ZIndex = ZIndex + 1
            end
            ...
            do
                Popup.Outline = Utility.New('Frame', {
                    Name = 'Outline', Size = UDim2.new(0, 0, 0, 0),
                    BorderSizePixel = 0, Parent = Library.ScreenGui, ZIndex = ZIndex,
                    ClipsDescendants = true, Visible = false,
                }, { BackgroundColor3 = 'Inline' })
                if self.BoundObjects then table.insert(self.BoundObjects, Popup.Outline) end
                Utility.New('UICorner', { Name = 'UICorner', Parent = Popup.Outline, CornerRadius = UDim.new(0, 5) })

                ZIndex = ZIndex + 1
                Popup.Background = Utility.New('TextButton', {
                    Name = 'Background', Size = UDim2.new(1, -2, 1, -2),
                    Position = UDim2.fromOffset(1, 1), Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    BorderSizePixel = 0, Parent = Popup.Outline, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Background' })
                Utility.New('UICorner', { Name = 'UICorner', Parent = Popup.Background, CornerRadius = UDim.new(0, 5) })
                Utility.New('UIPadding', {
                    Name = 'UIPadding', Parent = Popup.Background,
                    PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5),
                    PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5),
                })
                ...
                Popup.Scroll = Utility.New('ScrollingFrame', {
                    ...
                    Position = UDim2.fromOffset(0, 0), BackgroundTransparency = 1,
                    BorderSizePixel = 0, Parent = Popup.Background, ZIndex = ZIndex,
                    AutomaticCanvasSize = Enum.AutomaticSize.Y,
                    CanvasSize = UDim2.new(0, 0, 0, 0),
                    BottomImage = Library.ScrollBar, MidImage = Library.ScrollBar,
                    TopImage = Library.ScrollBar, ScrollBarThickness = 2,
                ...
                Popup.Holder = Utility.New('Frame', {
                    Name = 'Holder', Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, BorderSizePixel = 0,
                    BackgroundTransparency = 1, Parent = Popup.Scroll, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Parent = Popup.Holder, Padding = UDim.new(0, 5),
                })
            end
            ...
            function Dropdown.Display()
                ...
                if cfg.multi then
                    ...
                    if Value and #Value > 0 then
                        for _, item in Value do
                            if type(item) == 'string' then table.insert(CurrentText, item) end
                        end
                        Objects.Value.Text = #CurrentText > 0 and table.concat(CurrentText, ', ') or '-'
                    else
                        Objects.Value.Text = '-'
                    end
                else
            ...
            function Dropdown.Size()
                ...
                for _, v in Popup.Holder:GetChildren()do
                    Count = Count + 1
                    if v:IsA('TextButton') then Size = Size + v.AbsoluteSize.y + 5 end
            ...
            function Dropdown.Open()
                ...
                if Dropdown.Visible then Popup.Outline.Visible = true end
                ...
                for _, obj in ParentObjects do
                    local Index = Utility.GetTransparency(obj)
                    if type(Index) == 'table' then
                        for _, prop in Index do Library.Fade(obj, prop, Dropdown.Visible) end
                    else
            ...
            function Dropdown.Set(value, ignore)
                if cfg.multi then
                    if type(value) == 'table' then
                        for _, item in Dropdown.Items do item.Select(false) end
                        for _, item in value do
                        ...
                        Dropdown.Display()
                        if not ignore then cfg.callback(Dropdown.Value) end
            ...
            function Dropdown.Add(name)
                ...
                Objs.Text = Utility.New('TextButton', {
                    ...
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Popup.Holder, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
                function Item.Select(value)
                    Library.ChangeObjectTheme(Objs.Text, { TextColor3 = value and 'Text' or 'Dark Text' }, true)
                    Item.Selected = value
            ...
            function Dropdown.Refresh(tbl)
                for _, item in Dropdown.Items do item.Objects.Text:Destroy() end
                Dropdown.Items = {}
                Dropdown.Value = cfg.multi and {} or nil
                for _, item in tbl do
                    if type(item) == 'string' then Dropdown.Add(item) end
                end
                Dropdown.Display()
            end

            for _, item in cfg.values do Dropdown.Add(item) end
        ...
        function Library.Colorpicker(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Colorpicker', value = Color3.new(1, 1, 1),
                alpha = 0, flag = nil, callback = function() end,
            })

            local Colorpicker = {
                Tweening = false, ZIndex = self.ZIndex,
                Objects = {}, Popup = {}, Value = cfg.value, Alpha = cfg.alpha,
            }
            ...
            do
                ...
                Objects.Outline = Utility.New('TextButton', {
                    Name = 'Outline', Size = UDim2.new(0, 30, 0, 15),
                    Position = UDim2.fromOffset(0, 0), BorderSizePixel = 0,
                    BackgroundTransparency = 0, Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Parent = Parent, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Outline })
                ...
                Objects.Background = Utility.New('Frame', {
                    Name = 'Background', Size = UDim2.new(1, -2, 1, -2),
                    Position = UDim2.fromOffset(1, 1), BorderSizePixel = 0,
                    BackgroundTransparency = 0, Parent = Objects.Outline, ZIndex = ZIndex,
                })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Background })
                ZIndex = ZIndex + 1
            end
            ...
            function Colorpicker.Set(value, alpha)
                ...
                Library.Flags[cfg.flag] = { c = Color, a = Alpha }
                cfg.callback({ c = Color, a = Alpha })
            end
        ...
        function Library.Textbox(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Textbox', value = '',
                callback = function() end, flag = nil,
            })
            if not cfg.flag then cfg.flag = cfg.name end

            local Textbox = { ZIndex = self.ZIndex, Objects = {} }
            local ZIndex = Textbox.ZIndex
            local Objects = Textbox.Objects

            do
                Objects.Outline = Utility.New('TextButton', {
                    Name = 'Outline', Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, BorderSizePixel = 0,
                    Parent = self.Holder, TextSize = 0, Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    ZIndex = ZIndex, ClipsDescendants = true,
                }, { BackgroundColor3 = 'Inline' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Objects.Outline })
                ...
                Objects.TextBox = Utility.New('TextBox', {
                    Name = 'TextBox', Size = UDim2.new(1, 0, 0, 0),
                    Text = cfg.value, ClearTextOnFocus = false,
                    PlaceholderText = cfg.name, ClipsDescendants = true,
                    MultiLine = false, TextSize = Library.FontSize,
                    FontFace = Library.Font, TextXAlignment = Enum.TextXAlignment.Center,
                    BackgroundTransparency = 1, TextStrokeTransparency = 0.8,
                    Parent = Objects.Background, ZIndex = ZIndex,
        ...
        function Library.Keybind(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Keybind', value = false,
                key = Enum.KeyCode.Unknown, mode = 'Toggle',
                callback = function() end, flag = nil,
            })
            if not cfg.flag then cfg.flag = cfg.name end

            if not table.find(Library.KeybindFlags, cfg.flag) then
                table.insert(Library.KeybindFlags, cfg.flag)
            end

            local Keybind = {
                Tweening = false, Visible = false, ZIndex = self.ZIndex,
                Objects = {}, Popup = {}, Key = nil, Mode = nil,
                Value = false, OnHold = nil, Listener = nil,
            ...
            do
                Popup.Outline = Utility.New('Frame', {
                    Name = 'Outline', Size = UDim2.new(0, 150, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, BorderSizePixel = 0,
                    Parent = Library.ScreenGui, ZIndex = ZIndex,
                    ClipsDescendants = true, Visible = false,
                }, { BackgroundColor3 = 'Inline' })
                Utility.New('UICorner', { Name = 'UICorner', Parent = Popup.Outline, CornerRadius = UDim.new(0, 5) })
                ...
                Popup.Holder = Utility.New('Frame', {
                    Name = 'Holder', Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, BorderSizePixel = 0,
                    BackgroundTransparency = 1, Parent = Popup.Background, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                ...
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder, Parent = Popup.Line,
                    Padding = UDim.new(0, 2),
                })
                ...
                Popup.Text = Utility.New('TextLabel', {
                    ...
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(1, 0, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y, Text = 'Key',
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Popup.Line, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Horizontal,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    HorizontalAlignment = Enum.HorizontalAlignment.Right,
                    Parent = Popup.Text, Padding = UDim.new(0, 2),
                })
                ...
                Popup.KeyOutline = Utility.New('TextButton', {
                    Name = 'Outline', Size = UDim2.new(0, 0, 0, 0),
                    Position = UDim2.fromOffset(0, 0), BorderSizePixel = 0,
                    BackgroundTransparency = 0, Parent = Popup.Text, Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                Popup.KeyOutline.Size = UDim2.new(0, 25, 0, Popup.Text.AbsoluteSize.Y)
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Popup.KeyOutline })

                ZIndex = ZIndex + 1
                Popup.KeyBackground = Utility.New('Frame', {
                    Name = 'Background', Size = UDim2.new(1, -2, 1, -2),
                    Position = UDim2.fromOffset(1, 1), BorderSizePixel = 0,
                    Parent = Popup.KeyOutline, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Background' })
                Utility.New('UICorner', { Name = 'UICorner', CornerRadius = UDim.new(0, 5), Parent = Popup.KeyBackground })
                ...
                Popup.Key = Utility.New('TextLabel', {
                    ...
                    BackgroundTransparency = 1, TextSize = Library.FontSize - 2,
                    FontFace = Library.Font, Size = UDim2.new(1, 0, 1, -2),
                    Text = 'E', Parent = Popup.KeyBackground, ZIndex = ZIndex,
                }, { TextColor3 = 'Dark Text' })
            end
            ...
            function Keybind.Set(value, ignore)
                if type(value) == 'table' then
                    for _, v in value do Keybind.Set(v, true) end
                    return
                end
                ...
                elseif Type == 'boolean' then
                    if Keybind.Mode == 'Always' and not value then value = true end
                    Keybind.Value = value
                elseif Type == 'string' then
                    ...
                    Keybind.Mode = value
                    if Popup.Dropdown then Popup.Dropdown.Set(Keybind.Mode, true) end
                    if value == 'Always' then Keybind.Value = true end
                end
                if not ignore then
                ...
                if Library.KeybindList and type(Library.KeybindList) == 'table' and Library.KeybindList.Add then
                    Library.KeybindList.Add(cfg.flag, Keybind.DisplayName, Keybind.Key, Keybind.Mode)
                end
            end
            ...
            Popup.Dropdown = Library.Dropdown({
                ...
                Value = cfg.mode,
                Callback = function(v) Keybind.Set(v, true) end,
            })

            function Keybind.Open()
                if Keybind.Tweening then return end
                Keybind.Tweening = true
                Keybind.Visible = not Keybind.Visible
                if Keybind.Visible then Popup.Outline.Visible = true end
                ...
                for _, obj in ParentObjects do
                    ...
                    if Index then
                        if type(Index) == 'table' then
                            for _, prop in Index do Library.Fade(obj, prop, Keybind.Visible) end
                        else
            ...
            Utility.Signal(Popup.KeyOutline.MouseButton1Click:Connect(function(input)
                if Keybind.Listener then
                    Library.ChangeObjectTheme(Popup.Key, { TextColor3 = 'Dark Text' }, true)
                    Keybind.Listener:Disconnect()
                    Keybind.Listener = nil
                    return
                end
                Library.ChangeObjectTheme(Popup.Key, { TextColor3 = 'Accent' }, true)
                task.wait(2E-2)
            ...
            Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                if Keybind.Key ~= Enum.KeyCode.Unknown and (input.KeyCode == Keybind.Key or input.UserInputType == Keybind.Key) then
                    ...
                    if Keybind.Mode == 'Hold' then
                        if Keybind.OnHold then Keybind.OnHold:Disconnect() end
            ...
            Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 and Keybind.Visible and not (Utility.MouseOver(Popup.Outline, input) or Utility.MouseOver(Objects.Icon, input) or Utility.MouseOver(Popup.Dropdown.Popup.Outline, input)) then
                    if Keybind.Open then Keybind.Open() end
                end
            end))
            Utility.Signal(Objects.Icon.MouseButton1Click:Connect(function()
                if Keybind.Open then Keybind.Open() end
        ...
        function Library.Label(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, { name = 'New Label', bold = true, dark = false })
            local Label = { Objects = {}, ZIndex = self.ZIndex }
            ...
            do
                ...
                Objects.Text = Utility.New('TextLabel', {
                    ...
                    Text = cfg.name, TextWrapped = true,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.Holder, ZIndex = ZIndex,
        ...
        function Library.Notification(cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                name = 'New Notification', description = nil, type = 'Normal',
                buttons = nil, showbuttons = false, time = 5,
            })

            local ZIndex = 100
            local Notification = {
                Objects = {}, Name = cfg.name, Description = cfg.description,
                Type = cfg.type, Time = cfg.time, Clock = os.clock(),
                Buttons = {}, ShowButtons = cfg.showbuttons, ButtonLerp = 1, Lerp = 1,
            }
            local Objects = Notification.Objects

            do
                Objects.Holder = Utility.New('Frame', {
                    Name = 'Holder', BorderSizePixel = 0, BackgroundTransparency = 1,
                    Size = UDim2.new(0, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.XY,
                    ZIndex = ZIndex, Parent = Library.ScreenGui,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Parent = Objects.Holder, Padding = UDim.new(0, 5),
                })

                ZIndex = ZIndex + 1
                Objects.Outline = Utility.New('Frame', {
                    Name = 'Outline', BorderSizePixel = 0,
                    Size = UDim2.new(0, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.XY,
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding',
                    PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5),
                    PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 6),
                    Parent = Objects.Background,
                })
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.Background, CornerRadius = UDim.new(0, 5) })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Parent = Objects.Background, Padding = UDim.new(0, 5),
                })

                ZIndex = ZIndex + 1
                Objects.Title = Utility.New('TextLabel', {
                    Name = 'Title', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(0, 0, 0, 0),
                    Position = UDim2.new(0, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.XY,
                    Text = cfg.name, Parent = Objects.Background, ZIndex = ZIndex,
                }, { TextColor3 = 'Text' })
                ZIndex = ZIndex + 1
                ...
                if cfg.buttons then
                    Objects.FadeHolder = Utility.New('Frame', {
                        ...
                        AutomaticSize = Enum.AutomaticSize.XY, Parent = Objects.Holder,
                    })
                    Objects.ButtonHolder = Utility.New('Frame', {
                        Name = 'ButtonHolder', BorderSizePixel = 0,
                        BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 0),
                        AutomaticSize = Enum.AutomaticSize.XY, Parent = Objects.FadeHolder,
                    })
                    Utility.New('UIListLayout', {
                        Name = 'UIListLayout', FillDirection = Enum.FillDirection.Horizontal,
                        SortOrder = Enum.SortOrder.LayoutOrder,
                        Parent = Objects.ButtonHolder, Padding = UDim.new(0, 2),
                    })
                    ZIndex = ZIndex + 1
                end
                if cfg.description then
                    Objects.Description = Utility.New('TextLabel', {
                        ...
                        BackgroundTransparency = 1, TextSize = Library.FontSize - 2,
                        FontFace = Library.Font, Size = UDim2.new(0, 0, 0, 0),
                        AutomaticSize = Enum.AutomaticSize.XY, Text = cfg.description,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        Parent = Objects.Background, ZIndex = ZIndex, TextWrapped = true,
                    }, { TextColor3 = 'Light Text' })
                    ZIndex = ZIndex + 1
                end
            ...
            function Notification.Hide() Notification.Time = 0 end
            function Notification.Title(text) Objects.Title.Text = text end
            function Notification.Description(text) Objects.Description.Text = text end
            function Notification.ButtonVisiblity(visibility) Notification.ShowButtons = visibility end
            function Notification.CreateButton(bcfg)
                bcfg = bcfg or {}
                bcfg = Library.Config(bcfg, { text = 'undefined', callback = function() end })
        ...
        ---------------------------------------------------------------
        -- WATERMARK (upper-left of main GUI when attached)
        ---------------------------------------------------------------
        function Library.Watermark(cfg)
            ...
            local ZIndex = 100
            local Watermark = {
                Objects = {}, Visible = cfg.visible, Rate = cfg.rate, Text = cfg.text,
                ZIndex = ZIndex, Clock = os.clock(),
                AttachToGui = cfg.attachToGui, Draggable = cfg.draggable,
                Dragging = false, DragStart = nil, DragStartPos = nil, SavedPosition = nil,
            }
            ...
            do
                ...
                Utility.New('UIPadding', {
                    Name = 'UIPadding',
                    PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5),
                    PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 7),
                    Parent = Objects.Background,
                })
                ...
                Objects.Text = Utility.New('TextLabel', {
                    ...
                    AutomaticSize = Enum.AutomaticSize.X,
                    FontFace = Library.Font, Size = UDim2.new(0, 0, 0, 0),
                    Position = UDim2.new(0, 0, 0, 0), Text = Watermark.Text,
                    Parent = Objects.Background, ZIndex = ZIndex,
                }, { TextColor3 = 'Text' })

                Objects.DragHandle = Utility.New('TextButton', {
                    Name = 'DragHandle', BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0),
                    Text = '', AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Parent = Objects.Outline, ZIndex = ZIndex + 1,
                })
            end
            ...
            function Watermark.SetAttachToGui(attach)
                Watermark.AttachToGui = attach
                if attach then
                    Watermark.SavedPosition = Objects.Outline.Position
                elseif Watermark.SavedPosition then
                    Objects.Outline.Position = Watermark.SavedPosition
                end
            end
            function Watermark.SetDraggable(enabled)
                Watermark.Draggable = enabled
                Objects.DragHandle.Visible = enabled
            end

            Library.StartDragging(Objects.Outline, Objects.DragHandle, Watermark)

            local function FindMainWindow()
                for _, desc in ipairs(game:GetService('CoreGui'):GetDescendants())do
                    if desc.Name == 'Outline'
                        and desc:FindFirstChild('SideContainer')
                        and desc:FindFirstChild('PageHolder')
                    then
                        return desc
                    end
                end
            ...
            function Watermark.Think()
                ...
                if Watermark.AttachToGui and Watermark.Visible and not Watermark.Dragging then
                    local mainWindow = FindMainWindow()
                    if mainWindow then
                        -- UPPER LEFT: aligned to left edge, top of the main window
                        Objects.Outline.Position = UDim2.new(
                            0,
                            mainWindow.AbsolutePosition.X,
                            0,
                            mainWindow.AbsolutePosition.Y - Objects.Outline.AbsoluteSize.Y - 5
                        )
                    end
                end
            end
        ...
        ---------------------------------------------------------------
        -- KEYBIND LIST (mini-UI floating panel)
        ---------------------------------------------------------------
        function Library.KeybindList(cfg)
            ...
            local ZIndex = 100
            local KeybindList = {
                Objects = {}, Visible = cfg.visible, Draggable = cfg.draggable,
                Dragging = false, DragStart = nil, DragStartPos = nil,
                ZIndex = ZIndex, Entries = {}, Blacklist = cfg.blacklist, Name = cfg.name,
            }
            local Objects = KeybindList.Objects

            do
                Objects.Outline = Utility.New('Frame', {
                    Name = 'KeybindList', BorderSizePixel = 0,
                    Size = UDim2.new(0, 170, 0, 0),
                    AutomaticSize = Enum.AutomaticSize.Y,
                    Position = cfg.position,
                    Parent = Library.ScreenGui, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Inline' })
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.Outline, CornerRadius = UDim.new(0, 6) })
                Utility.New('UISizeConstraint', {
                    Name = 'MinSize', Parent = Objects.Outline,
                    MinSize = Vector2.new(140, 30),
                })
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.Background, CornerRadius = UDim.new(0, 6) })
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = Objects.TitleBar, CornerRadius = UDim.new(0, 6) })

                Utility.New('Frame', {
                    Name = 'CornerHider', BorderSizePixel = 0,
                    Size = UDim2.new(1, 0, 0, 8), Position = UDim2.new(0, 0, 1, -8),
                    Parent = Objects.TitleBar, ZIndex = ZIndex,
                }, { BackgroundColor3 = 'Page Background' })

                Objects.Title = Utility.New('TextLabel', {
                    Name = 'Title', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize,
                    FontFace = Library.Font, Size = UDim2.new(1, -30, 1, 0),
                    Position = UDim2.new(0, 8, 0, 0),
                    Text = cfg.name, TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = Objects.TitleBar, ZIndex = ZIndex + 1,
                }, { TextColor3 = 'Accent' })

                Objects.CloseBtn = Utility.New('TextButton', {
                    Name = 'CloseBtn', BackgroundTransparency = 1,
                    Size = UDim2.new(0, 22, 1, 0), Position = UDim2.new(1, -22, 0, 0),
                    Text = '×', TextSize = Library.FontSize + 2,
                    FontFace = Library.Font, AutoButtonColor = false,
                    Style = Enum.ButtonStyle.Custom,
                    Parent = Objects.TitleBar, ZIndex = ZIndex + 2,
                }, { TextColor3 = 'Dark Text' })

                Utility.Signal(Objects.CloseBtn.MouseButton1Click:Connect(function()
                ...
                Objects.Holder = Utility.New('Frame', {
                    ...
                    Size = UDim2.new(1, -10, 0, 0), Position = UDim2.new(0, 5, 0, 28),
                    AutomaticSize = Enum.AutomaticSize.Y,
                    Parent = Objects.Background, ZIndex = ZIndex,
                })
                Utility.New('UIListLayout', {
                    Name = 'UIListLayout', FillDirection = Enum.FillDirection.Vertical,
                    SortOrder = Enum.SortOrder.LayoutOrder,
                    Parent = Objects.Holder, Padding = UDim.new(0, 4),
                })
                Utility.New('UIPadding', {
                    Name = 'UIPadding', Parent = Objects.Holder,
                ...
                Objects.DragHandle = Utility.New('TextButton', {
                    Name = 'DragHandle', BackgroundTransparency = 1,
                    Size = UDim2.new(1, -22, 1, 0), Text = '',
                    AutoButtonColor = false, Style = Enum.ButtonStyle.Custom,
                    Parent = Objects.TitleBar, ZIndex = ZIndex + 1,
                    Visible = KeybindList.Draggable,
                })
            end

            KeybindList.ZIndex = ZIndex
            Library.StartDragging(Objects.Outline, Objects.DragHandle, KeybindList)
            ...
            local function GetKeyName(key)
                ...
                if typeof(key) == 'EnumItem' then
                    ...
                    local name = key.Name
                    return Library.KeyConverters[name:lower()] or name
                end
                if typeof(key) == 'UserInputType' then
                    local name = key.Name
                    return Library.KeyConverters[name:lower()] or name
                end
                return tostring(key)
            ...
            function KeybindList.Add(flag, displayName, key, mode)
                if table.find(KeybindList.Blacklist, flag) then return end

                if KeybindList.Entries[flag] then
                    KeybindList.Entries[flag].Set(displayName, key, mode)
                    return KeybindList.Entries[flag]
                end

                local Entry = { Flag = flag, Objects = {} }
                local E = Entry.Objects

                E.Frame = Utility.New('Frame', {
                    Name = 'Entry_' .. tostring(flag), BackgroundTransparency = 1,
                    BorderSizePixel = 0, Size = UDim2.new(1, 0, 0, 20),
                    Parent = Objects.Holder, ZIndex = ZIndex,
                })

                E.Name = Utility.New('TextLabel', {
                    Name = 'Name', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize - 2,
                    FontFace = Library.Font, Size = UDim2.new(1, -50, 1, 0),
                    Position = UDim2.new(0, 0, 0, 0), Text = displayName,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    TextTruncate = Enum.TextTruncate.AtEnd,
                    Parent = E.Frame, ZIndex = ZIndex,
                }, { TextColor3 = 'Light Text' })

                E.KeyOutline = Utility.New('Frame', {
                    Name = 'KeyOutline', BorderSizePixel = 0,
                    Size = UDim2.new(0, 0, 1, 0),
                    AutomaticSize = Enum.AutomaticSize.X,
                ...
                Utility.New('UICorner', { Name = 'UICorner', Parent = E.KeyOutline, CornerRadius = UDim.new(0, 4) })
                Utility.New('UIPadding', {
                    Name = 'UIPadding', Parent = E.KeyOutline,
                    PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 6),
                })

                E.Key = Utility.New('TextLabel', {
                    Name = 'Key', TextStrokeTransparency = 0.8,
                    BackgroundTransparency = 1, TextSize = Library.FontSize - 3,
                    FontFace = Library.Font, Size = UDim2.new(0, 0, 1, 0),
                    AutomaticSize = Enum.AutomaticSize.X,
                    Position = UDim2.new(0.5, 0, 0.5, 0),
                    AnchorPoint = Vector2.new(0.5, 0.5), Text = 'None',
                    TextXAlignment = Enum.TextXAlignment.Center,
                    Parent = E.KeyOutline, ZIndex = ZIndex + 1,
                }, { TextColor3 = 'Text' })

                function Entry.Set(name, key, mode)
                    E.Name.Text = name
                    E.Key.Text = GetKeyName(key) .. (mode == 'Hold' and ' ↓' or mode == 'Always' and ' ∞' or '')
                end

                KeybindList.Entries[flag] = Entry
                return Entry
            end

            function KeybindList.Remove(flag)
                local entry = KeybindList.Entries[flag]
                if entry then
                    entry.Objects.Frame:Destroy()
                    KeybindList.Entries[flag] = nil
                end
            end

            function KeybindList.Clear()
                for _, entry in pairs(KeybindList.Entries)do
                    entry.Objects.Frame:Destroy()
                end
                KeybindList.Entries = {}
            end

            function KeybindList.Rebuild()
                KeybindList.Clear()
                for _, flag in ipairs(Library.KeybindFlags)do
                    local data = Library.Flags[flag .. '_data']
                    if data and data.key then
                        local displayName = data.name or flag
                        KeybindList.Add(flag, displayName, data.key, data.mode)
                    end
                end
            end
            ...
            function KeybindList.SetDraggable(enabled)
                KeybindList.Draggable = enabled
                Objects.DragHandle.Visible = enabled
            end

            Utility.Signal(RunService.RenderStepped:Connect(function()
                Objects.Outline.Visible = KeybindList.Visible
            end))
        ...
        ---------------------------------------------------------------
        -- ESP PANEL (live character preview + box/name/health)
        ---------------------------------------------------------------
        function Library.ESPPanel(self, cfg)
            cfg = cfg or {}
            cfg = Library.Config(cfg, {
                height = 220,
                rotateSpeed = 0.5, -- degrees per frame-ish
                showBox = true,
                showName = true,
                showHealth = true,
                teamColor = Color3.fromRGB(0, 134, 229),
                nameColor = Color3.fromRGB(255, 255, 255),
                healthColor = Color3.fromRGB(0, 200, 83),
            })
            ...
            Objects.Container = Utility.New('Frame', {
                ...
                BorderSizePixel = 0,
                Size = UDim2.new(1, 0, 0, cfg.height),
                Parent = self.Holder,
                ClipsDescendants = true,
            })
            Utility.New('UICorner', {
                Name = 'UICorner', Parent = Objects.Container,
            ...
            -- Overlay
            Objects.Overlay = Utility.New('Frame', {
                Name = 'Overlay',
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 1, 0),
                Parent = Objects.Container,
            ...
            -- ESP Box (parented to Overlay, updated per-frame)
            Objects.Box = Utility.New('Frame', {
                Name = 'Box',
                BackgroundTransparency = 1,
                BorderSizePixel = 1,
                BorderColor3 = cfg.teamColor,
                Size = UDim2.new(0, 0, 0, 0),
                Position = UDim2.new(0.5, 0, 0.5, 0),
                AnchorPoint = Vector2.new(0.5, 0.5),
                Visible = cfg.showBox,
                Parent = Objects.Overlay,
            })
            ...
            Objects.Name = Utility.New('TextLabel', {
                ...
                TextXAlignment = Enum.TextXAlignment.Center,
                Visible = cfg.showName,
                Parent = Objects.Overlay,
            })

            -- ESP Health bar (left of box)
            Objects.HealthBG = Utility.New('Frame', {
                Name = 'HealthBG',
                ...
                Visible = cfg.showHealth,
                Parent = Objects.Overlay,
            })
            Utility.New('UICorner', {
                Name = 'UICorner', Parent = Objects.HealthBG,
                CornerRadius = UDim.new(1, 0),
            })

            Objects.HealthFill = Utility.New('Frame', {
                Name = 'HealthFill',
                BackgroundColor3 = cfg.healthColor,
                BorderSizePixel = 0,
                Size = UDim2.new(1, 0, 1, 0),
                AnchorPoint = Vector2.new(0, 1),
                Position = UDim2.new(0, 0, 1, 0),
                Parent = Objects.HealthBG,
            })
            Utility.New('UICorner', {
                Name = 'UICorner', Parent = Objects.HealthFill,
                CornerRadius = UDim.new(1, 0),
            })
            ...
            local function BuildModel()
                ...
                clone.Parent = Objects.Viewport

                for _, d in ipairs(clone:GetDescendants())do
                    if d:IsA('Script') or d:IsA('LocalScript')
                        or d:IsA('Sound') or d:IsA('ParticleEmitter') then
                        d:Destroy()
                ...
                local distance = size.Magnitude * 0.9
                local cam = Instance.new('Camera')
                cam.FieldOfView = 40
                cam.CFrame = CFrame.new(Vector3.new(0, 0, -distance), Vector3.new(0, 0, 0))
                cam.Parent = Objects.Viewport
                Objects.Viewport.CurrentCamera = cam
            ...
            Utility.Signal(LocalPlayer.CharacterAdded:Connect(function()
                task.wait(0.5)
                BuildModel()
            end))

            task.spawn(function()
                if LocalPlayer.Character then BuildModel() end
            end)

            -- Per-frame rotation + overlay sync
            local RS = game:GetService('RunService')
            local LastTick = tick()
            Utility.Signal(RS.RenderStepped:Connect(function()
                if not ESP.Running then return end
                ...
                ESP.Rotation = (ESP.Rotation + cfg.rotateSpeed * dt * 60) % 360

                local pivot = ESP.Model:GetPivot()
                local base = CFrame.new(Vector3.new(0, 0, 0))
                local distance = (ESP.ModelSize and ESP.ModelSize.Magnitude * 0.9) or 5
                local camCF = base
                    * CFrame.Angles(0, math.rad(ESP.Rotation), 0)
                    * CFrame.new(0, 0, -distance)
                    * CFrame.Angles(0, math.rad(180), 0)

                if ESP.Camera then
                    ESP.Camera.CFrame = camCF
                end
                ESP.Model:PivotTo(CFrame.new(0, 0, 0))

                -- Position overlay elements relative to the viewport center
                local overlaySize = Objects.Overlay.AbsoluteSize
                if overlaySize.X == 0 or overlaySize.Y == 0 then return end

                local boxHeight = overlaySize.Y * 0.75
                local boxWidth = boxHeight * 0.5
                local centerX = overlaySize.X / 2
                local centerY = overlaySize.Y / 2

                Objects.Box.Size = UDim2.fromOffset(boxWidth, boxHeight)
                Objects.Box.Position = UDim2.new(0.5, 0, 0.5, 0)

                Objects.Name.Position = UDim2.fromOffset(centerX, centerY - boxHeight / 2 - 4)

                Objects.HealthBG.Size = UDim2.fromOffset(4, boxHeight)
                Objects.HealthBG.Position = UDim2.fromOffset(centerX - boxWidth / 2 - 6, centerY)
                Objects.HealthFill.Size = UDim2.new(1, 0, 1, 0) -- full for now
            end))

            -- Setters (so you can hook colorpickers/toggles)
            function ESP.SetBoxVisible(v) Objects.Box.Visible = v end
            function ESP.SetNameVisible(v) Objects.Name.Visible = v end
            function ESP.SetHealthVisible(v)
                Objects.HealthBG.Visible = v
            end
            function ESP.SetBoxColor(c) Objects.Box.BorderColor3 = c end
            function ESP.SetNameColor(c) Objects.Name.TextColor3 = c end
            function ESP.SetHealthColor(c) Objects.HealthFill.BackgroundColor3 = c end
            function ESP.SetHeight(h)
                Objects.Container.Size = UDim2.new(1, 0, 0, h)
            end
            function ESP.SetRotateSpeed(s) cfg.rotateSpeed = s end

            table.insert(Library.ESPPanels, ESP)
            return ESP
        end

        ---------------------------------------------------------------
        -- Notification render loop
        ---------------------------------------------------------------
